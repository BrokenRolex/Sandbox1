...
class Emailer {
    ...
    def send (File file, String custFileName, Map summary, String emaulcsv) {
        def errors = 0
        try {
            Properties p = new Properties()
            p['mail.smtp.host'] = Props.instance.getProperty('mail.smtp.host','mail.mwh.com')
            p['mail.smtp.port'] = Props.instance.getProperty('mail.smtp.port','25')
            Session session = Session.getInstance(p)
            MimeMessage mimeMessage = new MimeMessage(session)
            mimeMessage.setSubject(email_subject(summary))
            def fromAddr = new InternetAddress(Props.instance.getProperty('email.from.default', 'do_not_reply@hdsupply.com'))
